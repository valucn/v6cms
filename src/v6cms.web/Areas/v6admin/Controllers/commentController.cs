...
namespace v6cms.web.Areas.v6admin.Controllers
{
    ...
    public class commentController : Controller
    {
        ...
        public commentController(IMemoryCache cache, db_context context)
        {
            _cache = cache;
            _context = context;
        }
        ...
        public async Task<IActionResult> index(int page = 1)
        {
            ...
            var model = await query.OrderByDescending(m => m.id).Include(m => m.article).ToPagedListAsync(page, 15);
            return View(model);
        }
        ...
        public async Task<IActionResult> edit(int? id)
        {
            ...
            var model = await _context.comment.Where(m => m.id == id).Include(m => m.article).FirstOrDefaultAsync();
            if (model == null)
            {
                return NotFound();
            }

...
namespace v6cms.web.Areas.v6admin.Controllers
{
    ...
    public class columnController : Controller
    {
        ...
        /// 修改栏目内容
        /// </summary>
        [admin_role_filter(authority_code = "column/edit_content")]
        public async Task<IActionResult> edit_content(int? id)
        {
            if (id == null)
            {
                return NotFound();
        ...
        [ValidateAntiForgeryToken]
        [admin_role_filter(authority_code = "column/edit_content")]
        public async Task<IActionResult> edit_content(int id, column_entity model)
        {
            if (id != model.id)
            {
                return NotFound();
